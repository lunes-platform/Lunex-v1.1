...
const redisModule = __importStar(require("../utils/redis"));
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const mockRedis = redisModule._mockRedis;
describe('buildWalletActionMessage', () => {
    it('builds a deterministic canonical message', () => {
        const msg = (0, auth_1.buildWalletActionMessage)({
            action: 'deposit',
        ...
        expect(msg).toBe('lunex-auth:deposit\n' +
            'address:5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY\n' +
            'amount:100\n' +
            'token:WLUNES\n' +
            'nonce:abc123\n' +
...
describe('verifyWalletActionSignature — replay protection', () => {
    beforeEach(() => {
        ...
        mockRedis.get.mockImplementation(async (key) => mockRedis._store.get(key) ?? null);
    ...
    it('marks nonce in Redis after successful use (mock set called)', async () => {
        ...
        await (0, auth_1.verifyWalletActionSignature)({
            action: 'trade',
            address: 'addr',
            nonce: 'fresh-nonce',
            timestamp: Date.now(),
        ...
        expect(mockRedis.set).not.toHaveBeenCalledWith(expect.stringContaining('fresh-nonce'), expect.anything(), expect.anything(), expect.anything());
    });
});
//# sourceMappingURL=redisNonce.test.js.map