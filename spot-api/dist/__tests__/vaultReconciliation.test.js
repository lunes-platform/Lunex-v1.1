...
function setupPrismaAggregates(deposited, withdrawn) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ;
    mockPrisma.copyVaultDeposit.aggregate.mockResolvedValue({
        _sum: { amount: deposited },
    });
    mockPrisma.copyVaultWithdrawal.aggregate.mockResolvedValue({
...
describe('VaultReconciliationService', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        mockPrisma.copyVault.update.mockResolvedValue({});
    });
    describe('isEnabled()', () => {
        it('returns true when config.reconciliation.enabled is true', () => {
            expect(vaultReconciliationService_1.vaultReconciliationService.isEnabled()).toBe(true);
        });
    });
    describe('runCycle()', () => {
        it('returns empty array when no vaults exist', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ;
            mockPrisma.copyVault.findMany.mockResolvedValue([]);
            const results = await vaultReconciliationService_1.vaultReconciliationService.runCycle();
            expect(results).toEqual([]);
        });
        it('returns a consistent result when equity matches (no drift)', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ;
            mockPrisma.copyVault.findMany.mockResolvedValue([makeVault({ totalEquity: '900' })]);
        ...
        it('detects drift and repairs vault when actual equity diverges', async () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ;
            mockPrisma.copyVault.findMany.mockResolvedValue([makeVault({ totalEquity: '500' })]);
            setupPrismaAggregates('1000', '100');
            const results = await vaultReconciliationService_1.vaultReconciliationService.runCycle();
            expect(results).toHaveLength(1);
        ...
        it('does not repair when drift is within the 0.01 threshold', async () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ;
            mockPrisma.copyVault.findMany.mockResolvedValue([makeVault({ totalEquity: '899.995' })]);
            setupPrismaAggregates('1000', '100');
            const results = await vaultReconciliationService_1.vaultReconciliationService.runCycle();
            expect(results[0].repaired).toBe(false);
            expect(mockPrisma.copyVault.update).not.toHaveBeenCalled();
        ...
        it('handles null aggregate sums (empty vault with no activity)', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ;
            mockPrisma.copyVault.findMany.mockResolvedValue([makeVault({ totalEquity: '0' })]);
            mockPrisma.copyVaultDeposit.aggregate.mockResolvedValue({ _sum: { amount: null } });
            mockPrisma.copyVaultWithdrawal.aggregate.mockResolvedValue({ _sum: { grossAmount: null } });
            const results = await vaultReconciliationService_1.vaultReconciliationService.runCycle();
            expect(results).toHaveLength(1);
        ...
        it('skips a cycle if the previous one is still running', async () => {
            ...
            let resolveFirst;
            const firstCallStarted = new Promise((res) => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ;
                mockPrisma.copyVault.findMany.mockImplementationOnce(() => new Promise((r) => {
                    resolveFirst = () => r([]);
                    res();
                }));
            });
            const firstCycle = vaultReconciliationService_1.vaultReconciliationService.runCycle();
            // Wait until first cycle is actually inside runCycle
            await firstCallStarted;
            // Second call should be skipped
            const secondCycle = await vaultReconciliationService_1.vaultReconciliationService.runCycle();
    ...
    describe('start() / stop()', () => {
        it('start() sets an interval and stop() clears it', () => {
            ...
            const runCycleSpy = jest.spyOn(vaultReconciliationService_1.vaultReconciliationService, 'runCycle').mockResolvedValue([]);
...
//# sourceMappingURL=vaultReconciliation.test.js.map