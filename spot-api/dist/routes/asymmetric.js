...
const asymmetricService_1 = require("../services/asymmetricService");
 ...
 */
router.get('/strategies', handleAsync(async (req, res) => {
    const { address } = req.query;
    if (!address)
        return send(res, { error: 'address query param is required' }, 400);
    const strategies = await asymmetricService_1.asymmetricService.listUserStrategies(address);
 ...
 */
router.post('/strategies', handleAsync(async (req, res) => {
    const { userAddress, pairAddress, agentId, isAutoRebalance, buyK, buyGamma, buyMaxCapacity, buyFeeTargetBps, sellGamma, sellMaxCapacity, sellFeeTargetBps, sellProfitTargetBps, leverageL, allocationC, } = req.body;
    ...
    const strategy = await asymmetricService_1.asymmetricService.createStrategy({
        userAddress,
        pairAddress,
        agentId,
        isAutoRebalance,
 ...
 */
router.get('/strategies/:id', handleAsync(async (req, res) => {
    const strategy = await asymmetricService_1.asymmetricService.getStrategy(req.params.id);
 ...
 */
router.patch('/strategies/:id/auto', handleAsync(async (req, res) => {
    const { address, enable } = req.body;
    if (!address || enable === undefined)
        return send(res, { error: 'address and enable are required' }, 400);
    const strategy = await asymmetricService_1.asymmetricService.toggleAutoRebalance(req.params.id, address, Boolean(enable));
    return send(res, strategy);
}));
/**
 ...
 */
router.patch('/strategies/:id/curve', handleAsync(async (req, res) => {
    ...
    const strategy = await asymmetricService_1.asymmetricService.updateCurveParams(req.params.id, address, {
 ...
 */
router.get('/strategies/:id/logs', handleAsync(async (req, res) => {
    const limit = Math.min(Number(req.query.limit ?? 50), 200);
    const logs = await asymmetricService_1.asymmetricService.getRebalanceLogs(req.params.id, limit);
 ...
 */
router.post('/agent/create-strategy', handleAsync(async (req, res) => {
    const { agentId, userAddress, pairAddress, buyK, buyGamma, buyMaxCapacity, buyFeeTargetBps, sellGamma, sellMaxCapacity, sellFeeTargetBps, sellProfitTargetBps, leverageL, allocationC, } = req.body;
 ...
 */
router.post('/agent/update-curve', handleAsync(async (req, res) => {
    ...
    const strategy = await asymmetricService_1.asymmetricService.updateCurveParams(strategyId, userAddress, {
        isBuySide: Boolean(isBuySide),
        newGamma: newGamma !== undefined ? Number(newGamma) : undefined,
        newMaxCapacity: newMaxCapacity !== undefined ? String(newMaxCapacity) : undefined,
        newFeeTargetBps: newFeeTargetBps !== undefined ? Number(newFeeTargetBps) : undefined,
 ...
 */
router.get('/agent/strategy-status/:id', handleAsync(async (req, res) => {
    const strategy = await asymmetricService_1.asymmetricService.getStrategy(req.params.id);
    const logs = await asymmetricService_1.asymmetricService.getRebalanceLogs(req.params.id, 10);
    return send(res, {
