...
describe('withTxTimeout', () => {
  ...
  it('rejects with TxTimeoutError when the promise takes too long', async () => {
    ...
    await expect(withTxTimeout('slow-tx', neverResolves, 50)).rejects.toThrow(
      TxTimeoutError,
    );
  });
  ...
  it('propagates rejection from the underlying promise', async () => {
    ...
    await expect(withTxTimeout('fail-tx', failing, 1000)).rejects.toThrow(
      'chain error',
    );
  });
