...
function makeVault(overrides: Partial<{ id: string; name: string; totalEquity: string; totalShares: string }> = {}) {
  return {
...
describe('VaultReconciliationService', () => {
  ...
  describe('runCycle()', () => {
    ...
    it('returns a consistent result when equity matches (no drift)', async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ;(mockPrisma.copyVault.findMany as jest.Mock).mockResolvedValue([makeVault({ totalEquity: '900' })])
    ...
    it('detects drift and repairs vault when actual equity diverges', async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ;(mockPrisma.copyVault.findMany as jest.Mock).mockResolvedValue([makeVault({ totalEquity: '500' })])
      setupPrismaAggregates('1000', '100')
    ...
    it('does not repair when drift is within the 0.01 threshold', async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ;(mockPrisma.copyVault.findMany as jest.Mock).mockResolvedValue([makeVault({ totalEquity: '899.995' })])
      setupPrismaAggregates('1000', '100')
    ...
    it('handles null aggregate sums (empty vault with no activity)', async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ;(mockPrisma.copyVault.findMany as jest.Mock).mockResolvedValue([makeVault({ totalEquity: '0' })])
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ;(mockPrisma.copyVaultDeposit.aggregate as jest.Mock).mockResolvedValue({ _sum: { amount: null } })
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ;(mockPrisma.copyVaultWithdrawal.aggregate as jest.Mock).mockResolvedValue({ _sum: { grossAmount: null } })
  ...
  describe('start() / stop()', () => {
    it('start() sets an interval and stop() clears it', () => {
      ...
      const runCycleSpy = jest.spyOn(vaultReconciliationService, 'runCycle').mockResolvedValue([])
