...
describe('calculateSharesToMint', () => {
  ...
  it('rejects non-positive deposits', () => {
    expect(() => calculateSharesToMint(0, 1000, 1000)).toThrow('Deposit amount must be positive')
  })
})
...
describe('calculateGrossWithdrawal', () => {
  ...
  it('rejects invalid share burns', () => {
    expect(() => calculateGrossWithdrawal(0, 1000, 4000)).toThrow('Shares to burn must be positive')
    expect(() => calculateGrossWithdrawal(1200, 1000, 4000)).toThrow('Cannot burn more shares than vault supply')
  })
})
...
describe('deriveAmountOut', () => {
  it('derives quote output for SELL flows', () => {
    expect(deriveAmountOut({ pairSymbol: 'LUNES/USDT', side: 'SELL', amountIn: 1000, executionPrice: 0.03 })).toBe(30)
  })

  it('derives base output for BUY flows', () => {
    expect(deriveAmountOut({ pairSymbol: 'LUNES/USDT', side: 'BUY', amountIn: 30, executionPrice: 0.03 })).toBeCloseTo(1000, 8)
  })
})
...
describe('presentation helpers', () => {
  ...
  it('formats member dates', () => {
    expect(formatMemberSince(new Date('2025-03-01T00:00:00Z'))).toBe('Mar 2025')
  })
})
