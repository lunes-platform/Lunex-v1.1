...
jest.mock('../../services/marginService', () => ({
  marginService: {
    getOverview: jest
      .fn()
      .mockResolvedValue({ account: null, positions: [], risk: {} }),
    ...
    resetPriceHealthMonitor: jest
      .fn()
      .mockReturnValue({ summary: {}, pairs: [] }),
...
jest.mock('../../services/agentService', () => ({
  agentService: {
    ...
    createApiKey: jest
      .fn()
      .mockResolvedValue({ id: 'key-1', key: 'api-key-123', prefix: 'lunx' }),
...
const verifyWalletActionSignatureMock =
  ...
  >;
const copytradeServiceMock = copytradeService as jest.Mocked<
...
describe('Auth attack simulation E2E', () => {
  ...
  it('blocks cross-agent API key listing when authenticated agent does not match target agent', async () => {
    ...
    expect(res.body).toHaveProperty(
      'error',
      'Authenticated agent does not match target agent',
    );
