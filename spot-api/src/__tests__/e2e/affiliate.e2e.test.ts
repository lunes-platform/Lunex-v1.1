...
jest.mock('../../services/affiliateService', () => ({
  affiliateService: {
    registerReferral: jest
      .fn()
      .mockImplementation(
        async (refereeAddress: string, referralCode: string) => ({
          id: 'ref-1',
          refereeAddress,
          referralCode,
        }),
      ),
    getOrCreateReferralCode: jest.fn().mockResolvedValue('ABCD1234'),
    ...
    getReferralTree: jest.fn().mockResolvedValue([]),
    getPayoutHistory: jest.fn().mockResolvedValue([]),
    processPayoutBatch: jest
      .fn()
      .mockResolvedValue({ batchId: 'batch-1', processed: 0, totalPaid: 0 }),
  },
}));
...
describe('Affiliate API E2E', () => {
  describe('POST /api/v1/affiliate/register', () => {
    ...
    it('should return 201 on valid registration', async () => {
      ...
      expect(res.body.referral).toHaveProperty(
        'refereeAddress',
        'new-user-addr',
      );
    });
