...
describe('Social API E2E', () => {
  ...
  describe('GET /api/v1/social/leaders', () => {
    it('should return leaders list with default params', async () => {
      const res = await request(app).get('/api/v1/social/leaders');
    ...
    it('should accept filter params', async () => {
      const res = await request(app).get(
        '/api/v1/social/leaders?tab=bots&sortBy=winRate&limit=10',
      );
  ...
  describe('GET /api/v1/social/leaderboard', () => {
    it('should return leaderboard', async () => {
      const res = await request(app).get('/api/v1/social/leaderboard');
  ...
  describe('GET /api/v1/social/ideas', () => {
    it('should return ideas list', async () => {
      const res = await request(app).get('/api/v1/social/ideas');
  ...
  describe('POST /api/v1/social/ideas/:ideaId/like', () => {
    ...
    it('should return 200 on valid like', async () => {
      const res = await request(app)
        .post('/api/v1/social/ideas/idea-1/like')
        .send(buildSignedBody({ address: 'test-address-123' }));
  ...
  describe('POST /api/v1/social/ideas/:ideaId/comments', () => {
    ...
    it('should return 201 on valid comment', async () => {
      const res = await request(app)
        .post('/api/v1/social/ideas/idea-1/comments')
        .send(
          buildSignedBody({ address: 'test-addr-123', content: 'Great idea!' }),
        );
  ...
  describe('GET /api/v1/social/following', () => {
    it('should return 400 without address', async () => {
      const res = await request(app).get('/api/v1/social/following');

      expect(res.status).toBe(400);
      expect(res.body).toHaveProperty('error', 'Validation failed');
    });

    it('should return 200 with signed address query', async () => {
      const res = await request(app)
  ...
  describe('POST /api/v1/social/leaders/:leaderId/follow', () => {
    ...
    it('should return 200 on valid follow', async () => {
      const res = await request(app)
        .post('/api/v1/social/leaders/leader-1/follow')
        .send(buildSignedBody({ address: 'follower-addr-123' }));
  ...
  describe('DELETE /api/v1/social/leaders/:leaderId/follow', () => {
    it('should return 400 without signed body', async () => {
      const res = await request(app).delete(
        '/api/v1/social/leaders/leader-1/follow',
      );
    ...
    it('should return 200 on valid unfollow', async () => {
      const res = await request(app)
        ...
        .send(buildSignedBody({ address: 'follower-addr' }));

      expect(res.status).toBe(200);
    });
  });

  describe('POST /api/v1/social/vaults/:leaderId/*', () => {
    it('should return 410 for deprecated deposit route', async () => {
      const res = await request(app)
        .post('/api/v1/social/vaults/leader-1/deposit')
        .send({});

      expect(res.status).toBe(410);
      expect(res.body).toHaveProperty('error', 'Deprecated route');
      expect(res.body).toHaveProperty('replacement.deposit');
    });

    it('should return 410 for deprecated withdraw route', async () => {
      const res = await request(app)
        .post('/api/v1/social/vaults/leader-1/withdraw')
        .send({});

      expect(res.status).toBe(410);
      expect(res.body).toHaveProperty('error', 'Deprecated route');
      expect(res.body).toHaveProperty('replacement.withdraw');
    });
  });
});
