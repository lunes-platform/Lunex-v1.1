...
export const executionLayerService = {
    ...
    async validateTrade(request: TradeRequest): Promise<ValidationResult> {
        ...
        if (!agent) return { allowed: false, checks, rejectionReason: 'Agent not found' }
        if (!agent.isActive) return { allowed: false, checks, rejectionReason: 'Agent is inactive' }
        if (agent.isBanned) return {
        ...
        const openOrderCount = await prisma.order.count({
            where: { makerAddress: (await prisma.agent.findUnique({ where: { id: agentId }, select: { walletAddress: true } }))!.walletAddress, status: { in: ['OPEN', 'PARTIAL'] } },
        })
            ...
            if (strategy) {
                ...
                if (vaultEquity > 0) {
                    ...
                    if (!checks.strategy_trade_size.passed) {
                        rejections.push(
                            `Trade size ${(tradeFraction * 100).toFixed(2)}% of vault equity exceeds ${strategy.riskLevel} limit ${(maxFraction * 100).toFixed(0)}%`,
                        )
                    }
                }
            }
    ...
    async getAgentExecutionHistory(agentId: string, opts: {
        status?: ExecutionStatus
        pairSymbol?: string
        since?: Date
        limit?: number
        ...
        const [entries, total] = await Promise.all([
            prisma.executionLog.findMany({ where, orderBy: { createdAt: 'desc' }, take: limit, skip: offset }),
    ...
    async getStrategyExecutionHistory(strategyId: string, opts: {
        status?: ExecutionStatus
        since?: Date
        limit?: number
        offset?: number
