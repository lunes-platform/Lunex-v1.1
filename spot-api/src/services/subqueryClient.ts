...
async function gql<T>(
  ...
  if (!response.ok) {
    throw new Error(
      `SubQuery GraphQL request failed: ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (body.errors?.length) {
    throw new Error(
      `SubQuery GraphQL errors: ${body.errors.map((e) => e.message).join(', ')}`,
    );
  }
...
export const subqueryClient = {
  ...
  // ── Swaps by wallet ──────────────────────────────────────────
  async getSwapsByAddress(
    address: string,
    limit = 500,
  ): Promise<SubquerySwapEvent[]> {
  ...
  async getVaultEventsByAddress(
    address: string,
    limit = 500,
  ): Promise<SubqueryVaultEvent[]> {
  ...
  async getTradeEventsByAddress(
    address: string,
    limit = 500,
  ): Promise<SubqueryTradeEvent[]> {
  ...
  async getWalletSummary(
    address: string,
  ): Promise<SubqueryWalletSummary | null> {
  ...
  async getDailyStats(days = 30): Promise<SubqueryDailyStats[]> {
    const data = await gql<{
      dailyProtocolStats: { nodes: SubqueryDailyStats[] };
    }>(
      `
