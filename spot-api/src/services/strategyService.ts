...
export const strategyService = {
    ...
    async createStrategy(input: CreateStrategyInput) {
        ...
        if (input.leaderId) {
            const leader = await prisma.leader.findUnique({ where: { id: input.leaderId } })
    ...
    async updateStrategy(id: string, agentId: string, input: UpdateStrategyInput) {
        const strategy = await prisma.strategy.findUnique({ where: { id } })
        if (!strategy) throw new Error('Strategy not found')
        if (strategy.agentId !== agentId) throw new Error('Unauthorized: not the strategy owner')
    ...
    async followStrategy(strategyId: string, followerAddress: string, allocatedCapital?: number) {
        ...
        if (!strategy) throw new Error('Strategy not found')
        if (strategy.status !== 'ACTIVE') throw new Error('Cannot follow a non-active strategy')
    ...
    async unfollowStrategy(strategyId: string, followerAddress: string) {
        ...
        if (!follow || !follow.isActive) throw new Error('Not following this strategy')
    ...
    async syncPerformanceFromLeader(strategyId: string) {
        const strategy = await prisma.strategy.findUnique({ where: { id: strategyId } })
        if (!strategy) throw new Error('Strategy not found')
        if (!strategy.leaderId) throw new Error('Strategy has no linked leader; cannot sync from leader analytics')
