...
export const affiliateService = {
    ...
    async _buildTreeBatched(address: string, currentDepth: number, maxDepth: number): Promise<unknown[]> {
        ...
        try {
            ...
            if (unpaid.length === 0) {
                await prisma.affiliatePayoutBatch.update({
                    where: { id: batch.id },
                    data: { status: 'COMPLETED', processedAt: now, totalPaid: new Decimal('0') },
                })
