...
export function calculateRoi(initialEquity: number, currentEquity: number): number {
...
export function getWindowRoi(points: EquityPoint[], days: number, referenceTime = Date.now()): number {
  if (points.length === 0) return 0
