...
import { buildSpotOrderMessage, verifyAddressSignature } from '../middleware/auth'
...
function resolveMethodKeyByKind(contract: ContractPromise, expectedLabel: string, kind: ContractMethodKind) {
  const expected = normalizeMethodKey(expectedLabel)
...
class SpotSettlementService {
  ...
  private async initialize() {
    try {
      ...
      const metadataPath = path.resolve(config.blockchain.spotContractMetadataPath)
      ...
      if (!settleMethodKey || !getBalanceMethodKey || !isNonceUsedMethodKey || !isNonceCancelledMethodKey) {
        log.warn('[SpotSettlement] Required contract methods not found in contract metadata')
        return false
      }
      ...
      log.error({ err: error }, '[SpotSettlement] Failed to initialize settlement service')
      return false
    }
  }
  ...
  async getVaultBalance(userAddress: string, tokenAddress: string, isNative: boolean): Promise<bigint | null> {
  ...
  async isNonceUsed(userAddress: string, nonce: string): Promise<boolean | null> {
    const isReady = await this.ensureReady()
    if (!isReady || !this.relayer) return null
  ...
  async isNonceCancelled(userAddress: string, nonce: string): Promise<boolean | null> {
    const isReady = await this.ensureReady()
    if (!isReady || !this.relayer) return null
  ...
  private toSignedOrder(pair: PairSettlementSnapshot, order: OrderSettlementSnapshot) {
    return {
  ...
  private buildOrderSignatureMessage(pair: PairSettlementSnapshot, order: OrderSettlementSnapshot) {
  ...
  private async assertOrderTrustedSource(pair: PairSettlementSnapshot, order: OrderSettlementSnapshot) {
  ...
  private async submitSettlement(input: TradeSettlementInput) {
    ...
    const queryMethod = (this.contract.query as Record<string, any>)[this.settleMethodKey]
    const txMethod = (this.contract.tx as Record<string, any>)[this.settleMethodKey]
    ...
    const txPromise = new Promise<string>((resolve, reject) => {
      ...
      txMethod(
        ...
        .signAndSend(this.relayer!, (txResult: { status: { isInBlock: boolean; isFinalized: boolean }; dispatchError?: { toString(): string }; txHash: { toHex(): string } }) => {
  ...
  async settleTrades(inputs: TradeSettlementInput[]): Promise<SettlementResult[]> {
    ...
    for (const input of inputs) {
      try {
        ...
        log.error({ err: error, tradeId: input.tradeId }, '[SpotSettlement] Failed to settle trade')
        settlements.push({
          tradeId: input.tradeId,
          status: 'FAILED',
  ...
  async cancelOrderFor(makerAddress: string, nonce: string): Promise<string | null> {
    const isReady = await this.ensureReady()
    if (!isReady || !this.relayer) return null

    const txMethod = this.getTxMethod(this.cancelOrderForMethodKey)
    if (!txMethod) {
      log.warn('[SpotSettlement] cancel_order_for method not found in contract metadata')
      return null
    }
    ...
    if (!queryMethod) {
      log.warn('[SpotSettlement] cancel_order_for query binding not found in contract metadata')
      return null
    }
    ...
    return withTxTimeout(`cancel_order_for:${makerAddress}:${nonce}`, cancelPromise)
  }
}
