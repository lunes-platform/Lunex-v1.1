...
const ROUTER_ABI_PATH = path.resolve(
  REPO_ROOT,
  'lunes-dex-main',
  'src',
  'abis',
...
const PAIR_ABI_PATH = path.resolve(
  REPO_ROOT,
  'lunes-dex-main',
  'src',
  'abis',
...
const WNATIVE_ABI_PATH = path.resolve(
  REPO_ROOT,
  'lunes-dex-main',
  'src',
  'abis',
...
function collectPrimitiveValues(
  ...
  if (typeof value === 'object') {
    Object.values(value).forEach((entry) =>
      collectPrimitiveValues(entry, output),
    );
  }
...
function normalizePlanckAmount(value: unknown) {
  ...
  return normalized / PLANCKS_PER_UNIT;
}
...
function deriveKind(section: string, method: string): IndexedEventKind {
  ...
  if (key.includes('tradeopen') || key.includes('positionopened'))
    return 'TRADE_OPEN';
  if (
    key.includes('tradeclose') ||
    key.includes('positionclosed') ||
...
function shouldTrackEvent(section: string, method: string) {
  ...
  if (
    pallet === 'balances' &&
    (eventName === 'deposit' || eventName === 'withdraw')
  ) {
    return false;
  }
...
function getBlockTimestamp(extrinsics: any[]) {
  const timestampExtrinsic = extrinsics.find((extrinsic) => {
    ...
    return (
      method?.section?.toString() === 'timestamp' &&
      method?.method?.toString() === 'set'
    );
  });
...
class SocialIndexerService {
  ...
  async getStatus() {
    const db = getAnalyticsDb();
    const cursor = db
      ? await db.socialAnalyticsCursor.findUnique({
          where: { chain: config.socialAnalytics.chainName },
        })
      : null;
  ...
  private async loadKnownContracts() {
    ...
    try {
      const [
        ...
        readJsonFile(ASYMMETRIC_PAIR_ABI_PATH).catch(() => null),
        prisma.asymmetricStrategy
          .findMany({
            select: { pairAddress: true },
            distinct: ['pairAddress'],
          })
          .catch(() => []),
      ]);
      ...
      if (
        typeof deployedAddresses.router === 'string' &&
        ACCOUNT_REGEX.test(deployedAddresses.router)
      ) {
      ...
      if (
        typeof deployedAddresses.wnative === 'string' &&
        ACCOUNT_REGEX.test(deployedAddresses.wnative)
      ) {
      ...
      for (const [key, value] of Object.entries(deployedAddresses)) {
        if (
          !key.toLowerCase().startsWith('pair') ||
          typeof value !== 'string' ||
          !ACCOUNT_REGEX.test(value)
        ) {
          continue;
        }
      ...
      if (copyVaultAbi) {
        for (const [key, value] of Object.entries(deployedAddresses)) {
          if (
            !key.toLowerCase().startsWith('copyvault') ||
            typeof value !== 'string' ||
            !ACCOUNT_REGEX.test(value)
          ) {
            continue;
      ...
      if (asymmetricPairAbi) {
        for (const entry of asymmetricStrategies as Array<{
          pairAddress: string;
        }>) {
          if (!ACCOUNT_REGEX.test(entry.pairAddress)) {
            continue;
          }

          this.knownContracts.set(entry.pairAddress, {
            kind: 'asymmetric_pair',
            contract: new ContractPromise(
              this.api as any,
              asymmetricPairAbi as any,
              entry.pairAddress,
            ),
          });
        }
      ...
      log.warn(
        { err: error },
        '[SocialIndexer] Failed to load known contract decoders',
      );
    }
  }
  ...
  private buildDecodedPayload(decoded: any) {
    ...
    const metadataArgs = (
      Array.isArray(decoded?.event?.args) ? decoded.event.args : []
    ) as any[];

    return metadataArgs.reduce(
      (acc: Record<string, unknown>, metaArg: any, index: number) => {
        const label = String(
          (metaArg as any)?.label ?? (metaArg as any)?.name ?? `arg${index}`,
        );
  ...
  private normalizeDecodedContractEvent(
    ...
    if (contractKind === 'router' && method === 'LiquidityAdded') {
      return {
        ...
        pairSymbol:
          this.getPairSymbolFromPath([payload.token_a, payload.token_b]) ||
          pairSymbol ||
          null,
    ...
    if (contractKind === 'router' && method === 'Swap') {
      return {
        ...
        counterpartyAddress: String(payload.to ?? ''),
        pairSymbol:
          this.getPairSymbolFromPath(payload.path) || pairSymbol || null,
    ...
    if (contractKind === 'pair' && method === 'Mint') {
      return {
        ...
        amountIn: Math.max(
          Number(payload.amount_0 ?? 0),
          Number(payload.amount_1 ?? 0),
        ),
        amountOut: Math.min(
          Number(payload.amount_0 ?? 0),
          Number(payload.amount_1 ?? 0),
    ...
    if (contractKind === 'pair' && method === 'Swap') {
      return {
        ...
        pairSymbol: pairSymbol || null,
        amountIn: Math.max(
          Number(payload.amount_0_in ?? 0),
          Number(payload.amount_1_in ?? 0),
        ),
        amountOut: Math.max(
          Number(payload.amount_0_out ?? 0),
          Number(payload.amount_1_out ?? 0),
        ),
        price: null,
        realizedPnl: null,
        payload,
    ...
    if (contractKind === 'copy_vault' && method === 'CircuitBreakerTriggered') {
      return {
        ...
        payload: {
          ...payload,
          drawdown_bps: Number(payload.drawdown_bps ?? 0),
        },
      };
    }

    if (
      contractKind === 'asymmetric_pair' &&
      method === 'AsymmetricSwapExecuted'
    ) {
      return {
        pallet: 'contracts.asymmetric_pair',
        method,
        kind: 'ASYMMETRIC_SWAP',
        accountAddress: String(payload.caller ?? signer ?? ''),
        counterpartyAddress: null,
        pairSymbol: pairSymbol || null,
        amountIn: normalizePlanckAmount(payload.amount_in),
        amountOut: normalizePlanckAmount(payload.liquidity_out),
        price: null,
        realizedPnl: null,
        payload: {
          ...payload,
          pairAddress: contractAddress ?? null,
          caller: String(payload.caller ?? signer ?? ''),
          liquidity_out: normalizePlanckAmount(payload.liquidity_out),
        },
      };
    }
  ...
  private tryDecodeContractEvent(
    ...
    try {
      const decoded = (decoder.contract.abi as any).decodeEvent(
        eventData as any,
      );
      const method = String(
        decoded?.event?.identifier ?? decoded?.event?.label ?? 'Unknown',
      );
      const payload = this.buildDecodedPayload(decoded);
      return this.normalizeDecodedContractEvent(
        decoder.kind,
        method,
        payload,
        signer,
      ...
      log.warn(
        { err: error, contractAddress },
        '[SocialIndexer] Failed to decode contract event',
      );
      return null;
    }
  }
  ...
  private normalizeEvent(
    ...
    return {
      ...
      counterpartyAddress:
        addresses.find((address) => address !== signer) || null,
  ...
  private isPrunedBlockError(error: unknown) {
    ...
    return (
      normalized.includes('state already discarded') ||
  ...
  private getRecoveryStartBlock(latestBlock: number) {
    return Math.max(
      latestBlock - config.socialAnalytics.maxBlocksPerRun + 1,
      1,
    );
  }
  ...
  private async processBlock(blockNumber: number) {
    ...
    for (
      let eventIndex = 0;
      eventIndex < eventRecordList.length;
      eventIndex += 1
    ) {
      ...
      const extrinsicIndex = phase?.isApplyExtrinsic
        ...
        : null;
      const extrinsic =
        extrinsicIndex != null ? extrinsics[extrinsicIndex] : null;
      const signer =
        extrinsic && (extrinsic as any).isSigned
          ? (extrinsic as any).signer.toString()
          : null;
      ...
      if (
        ...
        normalized = this.tryDecodeContractEvent(
          String(eventData[0]),
          eventData[1],
          signer,
        );
      ...
      const payload =
        normalized?.payload ??
        toSerializable(event.toHuman ? event.toHuman() : event.toJSON());
      ...
      if (normalized.kind === 'ASYMMETRIC_SWAP') {
        const pairAddress = String(
          (normalized.payload as any)?.pairAddress ?? '',
        );
        const acquiredAmount = normalized.amountOut ?? 0;

        if (ACCOUNT_REGEX.test(pairAddress) && acquiredAmount > 0) {
          try {
            await rebalancerService.handleCurveExecution(
              pairAddress,
              normalized.accountAddress ?? '',
              acquiredAmount,
            );
          } catch (error) {
            log.warn(
              { err: error, pairAddress, acquiredAmount },
              '[SocialIndexer] Failed to forward asymmetric swap to Sentinel',
            );
          }
        }
      }
  ...
  private async syncFromSubquery(leaderAddresses: string[]): Promise<{
    ...
    for (const address of leaderAddresses) {
      const { swaps, vaultEvents, tradeEvents } =
        await subqueryClient.getAllEventsByAddress(address, 1000);
      ...
      for (const vault of vaultEvents) {
        ...
        await db.socialIndexedEvent.create({
          data: {
            ...
            payload: {
              source: 'subquery',
              equityAfter: vault.equityAfter,
              drawdownBps: vault.drawdownBps,
            },
          },
        });
        totalIndexed += 1;
      ...
      for (const trade of tradeEvents) {
        ...
        const kind: IndexedEventKind =
          trade.kind === 'CLOSE' ? 'TRADE_CLOSE' : 'TRADE_OPEN';
        ...
        await db.socialIndexedEvent.create({
          data: {
            ...
            realizedPnl: trade.realizedPnl
              ? parseFloat(trade.realizedPnl) / 1e12
              : null,
  ...
  async syncOnce() {
    ...
    if (config.subquery.enabled && subqueryClient.isEnabled()) {
      try {
        ...
        log.info(
          ...
          '[SocialIndexer] SubQuery sync complete',
        );
        ...
        const message =
          error instanceof Error ? error.message : 'SubQuery sync failed';
        log.warn(
          { message },
          '[SocialIndexer] SubQuery sync failed, falling back to RPC',
        );
        await this.updateCursor({
          status: 'ERROR',
          lastError: `SubQuery: ${message}`,
        });
    ...
    if (!isReady || !this.api) {
      await this.updateCursor({
        status: 'ERROR',
        lastError: 'Blockchain API unavailable',
      });
      return {
        enabled: true,
        source: 'rpc',
    ...
    const startBlock =
      cursor.lastProcessedBlock > 0
        ...
        : Math.max(
            ...
            Math.max(
              latestBlock - config.socialAnalytics.backfillBlocks + 1,
              1,
            ),
          );
    ...
    const endBlock = Math.min(
      latestBlock,
      startBlock + config.socialAnalytics.maxBlocksPerRun - 1,
    );
    ...
    try {
      ...
      if (this.isPrunedBlockError(error)) {
        ...
        if (recoveryStartBlock > startBlock) {
          ...
          try {
            const recoveryEndBlock = Math.min(
              latestBlock,
              recoveryStartBlock + config.socialAnalytics.maxBlocksPerRun - 1,
            );
            const recoveredSummary = await this.processRange(
              recoveryStartBlock,
              recoveryEndBlock,
            );
            await this.updateCursor({ status: 'IDLE', lastError: null });
