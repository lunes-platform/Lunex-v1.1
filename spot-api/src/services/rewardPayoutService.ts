...
function resolveMethod(contract: ContractPromise, label: string, kind: 'tx' | 'query') {
...
class RewardPayoutService {
  ...
  private async initialize(): Promise<boolean> {
    try {
      ...
      const metadataPath = path.resolve(config.rewards.stakingContractMetadataPath)
      ...
      if (!this.fundMethodKey) {
        log.warn('[RewardPayout] fund_staking_rewards method not found in Staking ABI')
        return false
      }

      if (!this.distributeMethodKey && !this.distributePaginatedMethodKey) {
        log.warn('[RewardPayout] distribute_trading_rewards method not found in Staking ABI')
        return false
      }
  ...
  async getRelayerBalance(): Promise<bigint> {
    ...
    const { data: { free } } = await this.api.query.system.account(
  ...
  async fundStakingRewards(amountLunes: number): Promise<PayoutResult> {
    ...
    try {
      const queryMethod = (this.stakingContract.query as Record<string, any>)[this.fundMethodKey]
      const txMethod = (this.stakingContract.tx as Record<string, any>)[this.fundMethodKey]
      ...
      log.info({ txHash, amountLunes }, '[RewardPayout] Funded staking rewards')
      return { success: true, txHash, error: null }
    } catch (error) {
      const errMsg = error instanceof Error ? error.message : 'Unknown error'
      log.error({ err: error, amountLunes }, '[RewardPayout] fund_staking_rewards failed')
      return { success: false, txHash: null, error: errMsg }
    }
  }
   ...
   */
  async distributeRewards(startIndex?: number, batchSize?: number): Promise<PayoutResult> {
    const ready = await this.ensureReady()
    ...
    try {
      const queryMethod = (this.stakingContract.query as Record<string, any>)[methodKey]
      const txMethod = (this.stakingContract.tx as Record<string, any>)[methodKey]
      ...
      log.info({ txHash, startIndex, batchSize }, '[RewardPayout] Distributed trading rewards')
      return { success: true, txHash, error: null }
    } catch (error) {
      const errMsg = error instanceof Error ? error.message : 'Unknown error'
      log.error({ err: error }, '[RewardPayout] distribute_trading_rewards failed')
      return { success: false, txHash: null, error: errMsg }
    }
  }
   ...
   */
  async transferNative(toAddress: string, amountLunes: number): Promise<PayoutResult> {
    const ready = await this.ensureReady()
    ...
    try {
      const txPromise = new Promise<string>((resolve, reject) => {
        ...
        this.api!.tx.balances
          .transferKeepAlive(toAddress, new BN(amountPlancks.toString()))
      ...
      const txHash = await withTxTimeout(`transfer:${toAddress}:${amountLunes}`, txPromise)

      log.info({ txHash, toAddress, amountLunes }, '[RewardPayout] Native LUNES transferred')
      return { success: true, txHash, error: null }
    } catch (error) {
      const errMsg = error instanceof Error ? error.message : 'Unknown error'
      log.error({ err: error, toAddress, amountLunes }, '[RewardPayout] Native transfer failed')
      return { success: false, txHash: null, error: errMsg }
    }
  }
