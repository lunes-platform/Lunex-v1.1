...
const RATE_LIMITS_PER_TIER: Record<
  number,
  { maxPerHour: number; maxPerMinute: number }
> = {
...
interface CrossAgentTradeEntry {
  agentId: string;
  side: string;
  timestamp: number;
}
...
const recentTrades = new Map<
  string,
  Array<{ pairSymbol: string; side: string; amount: string; timestamp: number }>
>();

function recordTradeForAnalysis(
  agentId: string,
  trade: { pairSymbol: string; side: string; amount: string },
) {
  ...
  const filtered = trades.filter(
    (t) => now - t.timestamp < WASH_TRADE_WINDOW_MS,
  );
...
function recordCrossAgentTrade(
  agentId: string,
  trade: { pairSymbol: string; side: string; amount: string },
) {
  const key = `${trade.pairSymbol}:${trade.amount}`;
  const now = Date.now();
  const entries = (crossAgentRegistry.get(key) || []).filter(
    (e) => now - e.timestamp < COORDINATED_WASH_WINDOW_MS,
  );
  entries.push({ agentId, side: trade.side, timestamp: now });
  crossAgentRegistry.set(key, entries);
}

function detectCoordinatedWash(
  agentId: string,
  trade: { pairSymbol: string; side: string; amount: string },
): AnomalyFlag | null {
  const key = `${trade.pairSymbol}:${trade.amount}`;
  ...
  const counterpart = entries.find(
    (e) => e.agentId !== agentId && e.side === oppositeSide,
  );
  ...
  return {
    ...
    metadata: {
      ...
      counterpartTimestamp: counterpart.timestamp,
    },
  };
}
...
export function recordLargeOrderPlaced(
  agentId: string,
  orderId: string,
  amount: number,
...
export function recordOrderCancelled(
  agentId: string,
  orderId: string,
): AnomalyFlag | null {
  const now = Date.now();
  ...
  const idx = orders.findIndex(
    (o) =>
      o.orderId === orderId && now - o.placedAt < SPOOFING_CANCEL_WINDOW_MS,
  );
...
function detectAnomalies(
  ...
  const previousMinute = trades.filter(
    (t) => now - t.timestamp >= 60_000 && now - t.timestamp < 120_000,
  );

  if (
    previousMinute.length > 0 &&
    recentMinute.length / previousMinute.length > VELOCITY_SPIKE_THRESHOLD
  ) {
    flags.push({
      agentId,
      type: 'VELOCITY_SPIKE',
      ...
      timestamp: new Date(),
      metadata: {
        currentRate: recentMinute.length,
        previousRate: previousMinute.length,
      },
    });
  }
...
async function checkKeyRotation(
  agentId: string,
  keyId: string,
): Promise<{ needsRotation: boolean; daysUntilExpiry: number }> {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!allowed) {
      ...
      return res.status(429).json({
        ...
        limits:
          RATE_LIMITS_PER_TIER[req.agent.stakingTier] ||
          RATE_LIMITS_PER_TIER[0],
        retryAfterMs,
...
export function botAnomalyGuard() {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (flags.length > 0) {
      ...
      log.warn(
        {
          ...
          flags: flags.map((f) => `${f.type}(${f.severity})`),
          totalScore,
        },
        '[BotSandbox] Anomaly detected',
      );
      ...
      res.setHeader(
        'X-Anomaly-Warnings',
        JSON.stringify(flags.map((f) => f.type)),
      );
    }
...
export function keyRotationWarning() {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { needsRotation, daysUntilExpiry } = await checkKeyRotation(
      req.agent.id,
      req.agent.keyId,
    );
    if (needsRotation) {
      res.setHeader(
        'X-Key-Rotation-Warning',
        `API key expires in ${daysUntilExpiry} days. Please rotate.`,
      );
    }
...
export const botSandbox = {
  ...
  resetSpoofingCount: (agentId: string) =>
    largeOrderCancelCounts.delete(agentId),
