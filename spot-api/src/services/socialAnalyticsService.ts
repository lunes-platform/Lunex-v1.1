...
type IndexedEventKind = 'SWAP' | 'LIQUIDITY_ADD' | 'LIQUIDITY_REMOVE' | 'TRADE_OPEN' | 'TRADE_CLOSE' | 'VAULT_DEPOSIT' | 'VAULT_WITHDRAW' | 'UNKNOWN'
...
function buildEquityPoints(events: IndexedEventRecord[], fallbackEquity: number): EquityPoint[] {
  if (events.length === 0) {
...
function computeTradePnls(events: IndexedEventRecord[]) {
  return events
    .filter((event) => event.kind === 'TRADE_CLOSE' || (event.kind === 'SWAP' && toFiniteNumber(event.realizedPnl) !== 0))
...
function shouldDeduplicateEvent(event: IndexedEventRecord) {
  return Boolean(event.extrinsicHash) && (
    event.kind === 'SWAP' ||
...
function getTradeEventNotional(event: IndexedEventRecord) {
  return Math.max(toFiniteNumber(event.amountIn), toFiniteNumber(event.amountOut))
}
...
class SocialAnalyticsService {
  async recomputeLeaderSnapshots() {
    ...
    for (const leader of leaders) {
      ...
      const tradedVolume = roundMetric(
        events
          .filter((event: IndexedEventRecord) => event.kind === 'SWAP' || event.kind === 'TRADE_OPEN' || event.kind === 'TRADE_CLOSE')
          .reduce((total: number, event: IndexedEventRecord) => total + getTradeEventNotional(event), 0),
        6,
      )
      const grossProfit = roundMetric(tradePnls.filter((value) => value > 0).reduce((total, value) => total + value, 0), 6)
      const grossLoss = roundMetric(Math.abs(tradePnls.filter((value) => value < 0).reduce((total, value) => total + value, 0)), 6)
      const realizedPnl = roundMetric(tradePnls.reduce((total, value) => total + value, 0), 6)
  ...
  async getPipelineStatus() {
    ...
    const [indexedEvents, snapshots, latestIndexedEvent] = await Promise.all([
      db.socialIndexedEvent.count({ where: { chain: config.socialAnalytics.chainName } }),
      db.leaderAnalyticsSnapshot.count({ where: { sourceChain: config.socialAnalytics.chainName } }),
