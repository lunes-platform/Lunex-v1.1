...
const COPY_VAULT_ABI_PATH = path.resolve(
    __dirname, '../../../lunes-dex-main/src/abis/CopyVault.json',
)

const PLANCKS_PER_UNIT = new BN('100000000') // 10^8 for LUNES
...
class CopyVaultService {
    ...
    async deposit(
        ...
        const contract = new ContractPromise(this.api as any, this.abi, vaultAddress)
        const value = new BN(amount).mul(PLANCKS_PER_UNIT)
        ...
        const depositPromise = new Promise<VaultDepositResult>((resolve, reject) => {
        ...
        return withTxTimeout(`vault_deposit:${vaultAddress}:${depositorAddress}`, depositPromise)
    }
    ...
    async withdraw(
        ...
        const contract = new ContractPromise(this.api as any, this.abi, vaultAddress)
        const shareBN = new BN(shares).mul(PLANCKS_PER_UNIT)
        ...
        const withdrawPromise = new Promise<VaultWithdrawResult>((resolve, reject) => {
            let unsub: (() => void) | undefined
        ...
        return withTxTimeout(`vault_withdraw:${vaultAddress}:${withdrawerAddress}`, withdrawPromise)
    }
}
