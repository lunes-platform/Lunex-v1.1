...
const CreateApiKeySchema = z.object({
    ...
    permissions: z.array(
        z.enum(['TRADE_SPOT', 'TRADE_MARGIN', 'SOCIAL_POST', 'COPYTRADE_SIGNAL', 'READ_ONLY', 'MANAGE_ASYMMETRIC']),
    ).min(1),
...
const ListAgentsSchema = z.object({
    agentType: z.enum(['HUMAN', 'AI_AGENT', 'OPENCLAW_BOT', 'ALGO_BOT']).optional(),
    isActive: z.coerce.boolean().optional(),
    sortBy: z.enum(['totalTrades', 'totalVolume', 'stakedAmount', 'createdAt']).optional(),
...
router.get('/by-wallet/:address', async (req: Request, res: Response, next: NextFunction) => {
    try {
...
router.post('/:id/api-keys', optionalAgentAuth(), async (req: Request, res: Response, next: NextFunction) => {
    try {
...
router.delete('/:id/api-keys/:keyId', agentAuth(), async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!ensureAgentScope(req, res)) return
...
router.get('/:id/api-keys', agentAuth(), async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!ensureAgentScope(req, res)) return
...
router.post('/:id/stake', agentAuth(), async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!ensureAgentScope(req, res)) return
...
router.get('/:id/permissions', agentAuth(), async (req: Request, res: Response) => {
    if (!ensureAgentScope(req, res)) return
    res.json({
