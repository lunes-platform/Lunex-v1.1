...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const parsed = MarginOverviewQuerySchema.safeParse(req.query)
    if (!parsed.success) {
...
router.get('/price-health', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/price-health/reset', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const parsed = MarginPriceHealthResetSchema.safeParse(req.body)
    if (!parsed.success) {
      return res.status(400).json({ error: 'Validation failed', details: parsed.error.issues })
    }
    const status = marginService.resetPriceHealthMonitor(parsed.data.pairSymbol)
    res.json(status)
  } catch (err) { next(err) }
})
...
router.post('/collateral/deposit', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/collateral/withdraw', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const parsed = MarginCollateralSchema.safeParse(req.body)
    if (!parsed.success) {
      return res.status(400).json({ error: 'Validation failed', details: parsed.error.issues })
...
router.post('/positions/:id/close', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const overview = await marginService.closePosition(req.params.id, parsed.data.address)
    res.json(overview)
  } catch (err) { next(err) }
})

router.post('/positions/:id/liquidate', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const overview = await marginService.liquidatePosition(req.params.id, parsed.data.liquidatorAddress)
    res.json(overview)
  } catch (err) { next(err) }
})
