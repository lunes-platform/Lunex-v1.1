...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { status, orderId, executedAmount, executionPrice, slippageBps } =
        req.body;
      if (!status || !StatusValues.includes(status)) {
        return res
          .status(400)
          .json({ error: `status must be one of: ${StatusValues.join(', ')}` });
      }

      const entry = await executionLayerService.updateExecutionStatus(
        req.params.logId,
        {
          status,
          orderId,
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (strategyId) {
        const result = await executionLayerService.getStrategyExecutionHistory(
          req.agent!.id,
          strategyId,
          {
    ...
    try {
      const dateParam =
        typeof req.query.date === 'string'
          ...
          : undefined;
      const summary = await executionLayerService.getDailySummary(
        req.agent!.id,
        dateParam,
      );
      res.json({ summary });
    ...
    try {
      ...
      if (strategyId) {
        const strategy = await prisma.strategy.findFirst({
          where: { id: strategyId, agentId: agent.id },
        ...
        if (strategy) {
          params.strategy = {
            ...
            slippageCap: { LOW: 50, MEDIUM: 150, HIGH: 300, AGGRESSIVE: 500 }[
              strategy.riskLevel
            ],
            maxTradeSizePct: { LOW: 5, MEDIUM: 10, HIGH: 20, AGGRESSIVE: 40 }[
              strategy.riskLevel
            ],
          };
        }
