...
const MarketplaceSchema = z.object({
  ...
  search: z.string().max(200).optional(),
  sortBy: z
    .enum(['roi30d', 'followersCount', 'totalVolume', 'sharpeRatio'])
    .optional(),
  limit: z.coerce.number().int().min(1).max(100).optional(),
  offset: z.coerce.number().int().min(0).optional(),
});
...
const FollowSchema = SignedActionSchema.extend({
  followerAddress: z.string().min(8).max(128),
  allocatedCapital: z.coerce.number().nonnegative().optional(),
});

const UnfollowSchema = SignedActionSchema.extend({
  followerAddress: z.string().min(8).max(128),
});

const FollowedQuerySchema = SignedActionSchema;
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await strategyService.getFollowers(
        req.params.id,
        limit,
        offset,
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const followed = await strategyService.getFollowedStrategies(
        req.params.address,
      );
      res.json({ followed });
    } catch (err) {
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        err.message?.includes('Cannot follow') ||
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!auth.ok) return res.status(401).json({ error: auth.error });
      const result = await strategyService.unfollowStrategy(
        req.params.id,
        parsed.data.followerAddress,
      );
      res.json(result);
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (strategy.agentId !== req.agent!.id) {
        return res
          .status(403)
          .json({ error: 'Unauthorized: not the strategy owner' });
      }
      const result = await strategyService.syncPerformanceFromLeader(
        req.params.id,
      );
      if (!result)
        return res
          .status(404)
          .json({ error: 'No analytics snapshot found for linked leader' });
      res.json(result);
    } catch (err: any) {
      if (err.message === 'Strategy not found')
        return res.status(404).json({ error: err.message });
