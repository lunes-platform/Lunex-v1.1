...
const Tab = styled.button<{ active?: boolean }>`
  ...
  border-bottom: 2px solid
    ${({ active }) => (active ? '#6c38fe' : 'transparent')};
  ...
  const tradeHistory = useMemo(() => {
    if (isConnected && userTrades.length > 0) {
      return userTrades.map(t => {
        const isMaker =
          t.makerAddress === t.takerAddress ? false : t.makerFee !== '0'
        ...
        {activeTab === 'open' &&
          (openOrders.length > 0 ? (
                ...
                {openOrders.map(o => (
                  <tr key={o.id}>
                    ...
                    <td>
                      <CancelBtn
                        onClick={async () => {
                          await handleCancel(String(o.id))
                        }}
                      >
                        Cancel
                      </CancelBtn>
            ...
            <EmptyState>
              ...
              <span>
                {!isConnected
                  ? 'API offline — backend unavailable'
                  : !walletAddress
                ...
                {tradeHistory.map(t => (
                  <tr key={t.id}>
                    ...
                    <FeeCell>
                      ...
                      <FeeTooltip>
                        <FeeTooltipTitle>
                          Fee split ({t.isMaker ? 'Maker' : 'Taker'})
                        </FeeTooltipTitle>
                        <FeeTooltipRow>
                          <span>Team</span>
                          <span>{t.breakdown.team.toFixed(5)}</span>
                        </FeeTooltipRow>
                        <FeeTooltipRow>
                          <span>Stakers</span>
                          <span>{t.breakdown.stakers.toFixed(5)}</span>
                        </FeeTooltipRow>
                        <FeeTooltipRow>
                          <span>Affiliates</span>
                          <span>{t.breakdown.affiliates.toFixed(5)}</span>
                        </FeeTooltipRow>
                        {t.breakdown.treasury > 0 && (
                          <FeeTooltipRow>
                            <span>Treasury</span>
                            <span>{t.breakdown.treasury.toFixed(5)}</span>
                          </FeeTooltipRow>
                        )}
                      </FeeTooltip>
                    </FeeCell>
