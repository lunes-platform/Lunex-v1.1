...
import strategyService, { Strategy, StrategyType, StrategyRiskLevel } from '../../services/strategyService'
...
const Avatar = styled.div<{ url?: string }>`
  ...
  height: 30px;
  border-radius: 50%;
  background: ${({ url }) => url ? `url(${url}) center/cover` : 'linear-gradient(135deg, #7461FF, #26D07C)'};
  flex-shrink: 0;
`
...
const FollowBtn = styled.button<{ following?: boolean }>`
  ...
  background: ${({ following }) => following ? 'transparent' : 'rgba(116,97,255,0.12)'};
  ...
  transition: all 0.15s ease;
  &:hover:not(:disabled) {
    background: ${({ following }) => following ? 'rgba(255,77,77,0.08)' : 'rgba(116,97,255,0.22)'};
...
const LoadMoreBtn = styled.button`
  ...
  &:hover:not(:disabled) { border-color: #7461FF; color: #7461FF; }
...
const OnboardBanner = styled.div`
  ...
  gap: 16px;
  flex-wrap: wrap;
  background: linear-gradient(135deg, rgba(108,56,255,0.1) 0%, rgba(77,172,255,0.06) 100%);
...
const OnboardCTA = styled.button`
  ...
  white-space: nowrap;
  transition: all 0.15s;
  &:hover { background: rgba(108,56,255,0.3); color: #FFF; }
`
...
const StrategyMarketplacePage: React.FC = () => {
    ...
    const [sortBy, setSortBy] = useState<'roi30d' | 'followersCount' | 'sharpeRatio' | 'totalVolume'>('roi30d')
    const [searchQuery, setSearchQuery] = useState('')
    const [debouncedSearch, setDebouncedSearch] = useState('')
    const [following, setFollowing] = useState<Set<string>>(new Set())
    const [followLoading, setFollowLoading] = useState<Set<string>>(new Set())
    const [bannerDismissed, setBannerDismissed] = useState(() => sessionStorage.getItem('lunex_strat_banner') === '1')
    ...
    const loadMore = async () => {
        ...
        setLoadingMore(true)
        try {
            const result = await strategyService.getMarketplace(buildParams(nextOffset))
    ...
    useEffect(() => {
        const wallet = (window as any).__lunex_wallet
        if (!wallet) return
        strategyService.getFollowedStrategies(wallet)
    ...
    const handleFollow = async (strategyId: string, e: React.MouseEvent) => {
        ...
        try {
            if (following.has(strategyId)) {
                await strategyService.unfollowStrategy(strategyId, wallet)
                setFollowing((prev) => { const s = new Set(prev); s.delete(strategyId); return s })
                setStrategies((prev) => prev.map((s) => s.id === strategyId
                    ? { ...s, followersCount: Math.max(0, s.followersCount - 1) }
                    : s
                ))
            } else {
                await strategyService.followStrategy(strategyId, wallet)
            ...
            setFollowLoading((prev) => { const s = new Set(prev); s.delete(strategyId); return s })
        }
    }
        ...
        <Page>
            ...
            <Container>
                <Hero>
                    ...
                    <Subtitle>
                        Discover, follow, and allocate capital to algorithmic &amp; AI-powered trading strategies.
                        Built on Lunex Protocol — copy vaults, spot settlement, and AMM pools.
                    </Subtitle>
                    <button
                ...
                {!bannerDismissed && (
                    <OnboardBanner>
                        ...
                        <OnboardCTA onClick={() => navigate('/agents/get-started')}>Learn how &#8594;</OnboardCTA>
                        <OnboardCTA onClick={() => navigate('/agent')}>Create Agent</OnboardCTA>
                        <OnboardClose onClick={() => { setBannerDismissed(true); sessionStorage.setItem('lunex_strat_banner','1') }}>&#10005;</OnboardClose>
                    </OnboardBanner>
                )}
                ...
                <StatsBar>
                    ...
                    <StatCard>
                        <StatCardValue style={{ color: avgRoi30d >= 0 ? '#26D07C' : '#FF4D4D' }}>
                            {fmtRoi(avgRoi30d)}
                        </StatCardValue>
                ...
                <FiltersRow>
                    ...
                    />
                    <FilterSelect value={riskFilter} onChange={(e) => setRiskFilter(e.target.value as any)}>
                ...
                {loading ? (
                    ...
                    <Grid>
                        {strategies.length === 0 ? (
                            <RichEmptyState>
                                <EmptyIcon>
                                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#6C38FF" strokeWidth="2">
                                ...
                                <EmptyDesc>
                                    Be the first to publish a trading strategy on Lunex.
                                    Create an Agent, generate an API Key, and register your strategy to the marketplace.
                                </EmptyDesc>
                                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
                                    <EmptyCTA onClick={() => navigate('/agents/get-started')}>Get Started Guide</EmptyCTA>
                                    <button
                                        onClick={() => navigate('/agent')}
                                        style={{ padding:'12px 24px', borderRadius:12, border:'1px solid #2A2A2C', background:'transparent', color:'#8A8A8E', fontFamily:"'Space Grotesk'", fontSize:14, fontWeight:600, cursor:'pointer' }}
                                    >Create Agent</button>
                                </div>
                            </RichEmptyState>
                        ) : strategies.map((s) => {
                                ...
                                <Card key={s.id} onClick={() => navigate(`/strategies/${s.id}`)}>
                                    <CardHeader>
                                        <div>
                                            ...
                                            <CardMeta>
                                                <Chip color={TYPE_COLORS[s.strategyType]} bg={TYPE_COLORS[s.strategyType] + '18'}>
                                                ...
                                                {s.leader?.isAi && (
                                                    <Chip color="#7461FF" bg="rgba(116,97,255,0.12)">AI</Chip>
                                                )}
                                                {s.leader?.isVerified && (
                                                    <Chip color="#26D07C" bg="rgba(38,208,124,0.1)">✓ Verified</Chip>
                                                )}
                                            </CardMeta>
                                        </div>
                                        <ROIBadge positive={s.roi30d >= 0}>{fmtRoi(s.roi30d)}</ROIBadge>
                                    </CardHeader>

                                    {s.description && (
                                        <div style={{ color: '#8A8A8E', fontSize: '13px', fontFamily: 'Space Grotesk', lineHeight: 1.5 }}>
                                        ...
                                        <AgentRow>
                                            ...
                                            <AgentName>
                                                ...
                                                {s.agent?.agentType !== 'HUMAN' && (
                                                    <span style={{ marginLeft: 6, color: '#7461FF', fontSize: '11px' }}>
                                                        {s.agent?.framework ?? s.agent?.agentType}
                                                    </span>
                                                )}
                                            </AgentName>
                                            {s.agent && (
                                                <span style={{ marginLeft: 'auto', color: '#8A8A8E', fontSize: '12px' }}>
                ...
                {!loading && strategies.length < total && (
                    <LoadMoreBtn onClick={loadMore} disabled={loadingMore}>
                        {loadingMore ? 'Loading…' : `Load More (${total - strategies.length} remaining)`}
                    </LoadMoreBtn>
                )}
            </Container>
        </Page>
