...
import strategyService, { Strategy, StrategyPerformancePoint } from '../../services/strategyService'
...
const ROISubValue = styled.div<{ positive?: boolean }>`
  ...
  font-size: 13px;
  font-weight: 600;
  color: ${({ positive }) => positive === undefined ? '#FFFFFF' : positive ? '#26D07C' : '#FF4D4D'};
`
...
const ChartTab = styled.button<{ active?: boolean }>`
  background: ${({ active }) => active ? 'rgba(116,97,255,0.14)' : 'transparent'};
  ...
  &:hover:not([disabled]) { border-color: #7461FF; color: #9983FF; }
`
...
function CustomTooltip({ active, payload, label }: any) {
    ...
    return (
        <TooltipBox>
            ...
            <div style={{ color: p.roi >= 0 ? '#26D07C' : '#FF4D4D' }}>ROI {fmtRoi(p.roi)}</div>
...
const StrategyDetail: React.FC = () => {
    ...
    const reloadHistory = useCallback(async (days: DayRange) => {
        if (!id) return
        setDayRange(days)
        try {
    ...
    const handleFollow = async () => {
        if (!strategy) return
        const wallet = (window as any).__lunex_wallet
        if (!wallet) { alert('Connect your wallet to follow strategies.'); return }
        setFollowLoading(true)
        try {
            if (following) {
                await strategyService.unfollowStrategy(strategy.id, wallet)
                setFollowing(false)
                setStrategy((s) => s ? { ...s, followersCount: Math.max(0, s.followersCount - 1) } : s)
            } else {
                await strategyService.followStrategy(strategy.id, wallet)
                setFollowing(true)
                setStrategy((s) => s ? { ...s, followersCount: s.followersCount + 1 } : s)
            }
        } catch (e: any) {
            alert(e.message)
        } finally {
    ...
    if (error || !strategy) {
        return (
            <Page>
                <Container>
                    <BackBtn onClick={() => navigate('/strategies')}>← Back to Marketplace</BackBtn>
    ...
    const latestRoi = history.length > 0 ? history[history.length - 1].roi : strategy.roi30d
                ...
                <TopGrid>
                    <div>
                        <ChipRow>
                            ...
                            <Chip color={risk.color} bg={risk.bg}>{strategy.riskLevel}</Chip>
                            <Chip color={strategy.status === 'ACTIVE' ? '#26D07C' : '#FE923F'}
                                  bg={strategy.status === 'ACTIVE' ? 'rgba(38,208,124,0.1)' : 'rgba(254,146,63,0.1)'}>
                                {strategy.status}
                            </Chip>
                            {strategy.leader?.isAi && (
                                <Chip color="#7461FF" bg="rgba(116,97,255,0.12)">AI Agent</Chip>
                            )}
                            {strategy.leader?.isVerified && (
                                <Chip color="#26D07C" bg="rgba(38,208,124,0.08)">✓ Verified</Chip>
                            )}
                        </ChipRow>
                    ...
                    <ROIPanel>
                        ...
                        <ROISub>
                            <ROISubItem>
                                <ROISubValue positive={strategy.roi7d >= 0}>{fmtRoi(strategy.roi7d)}</ROISubValue>
                                <ROISubLabel>7d</ROISubLabel>
                            </ROISubItem>
                            <ROISubItem>
                                <ROISubValue positive={strategy.roi1d >= 0}>{fmtRoi(strategy.roi1d)}</ROISubValue>
                                <ROISubLabel>1d</ROISubLabel>
                            </ROISubItem>
                            <ROISubItem>
                                <ROISubValue>{strategy.followersCount.toLocaleString()}</ROISubValue>
                        ...
                        >
                            {followLoading ? '…' : following ? 'Following · Unfollow' : 'Follow Strategy'}
                        </FollowBtn>
                    </ROIPanel>
                </TopGrid>
                ...
                <Section>
                    ...
                    <ChartWrap>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                            <ChartTabRow>
                                <ChartTab active={chartMetric === 'equity'} onClick={() => setChartMetric('equity')}>
                                    Equity Curve
                                </ChartTab>
                                <ChartTab active={chartMetric === 'roi'} onClick={() => setChartMetric('roi')}>
                                    Daily ROI
                                </ChartTab>
                            </ChartTabRow>
                            <ChartTabRow>
                                {([7, 30, 90] as DayRange[]).map((d) => (
                                    <ChartTab key={d} active={dayRange === d} onClick={() => reloadHistory(d)}>
                                        {d}D
                                    </ChartTab>
                                ))}
                            </ChartTabRow>
                        ...
                        {chartData.length === 0 ? (
                            <div style={{ height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8A8A8E', fontFamily: 'Space Grotesk', fontSize: '14px' }}>
                            ...
                            <ResponsiveContainer width="100%" height={220}>
                                <AreaChart data={chartData} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
                                    ...
                                    <XAxis
                                        ...
                                        tick={{ fill: '#8A8A8E', fontSize: 11, fontFamily: 'Space Grotesk' }}
                                        axisLine={false}
                                        tickLine={false}
                                    />
                                    <YAxis
                ...
                {(strategy.agent || strategy.leader) && (
                    <Section>
                        ...
                        <AgentCard>
                                ...
                                <AgentMeta>
                                    ...
                                    {strategy.agent && (
                                        <span style={{ color: '#7461FF' }}>
                                            Reputation {Number(strategy.agent.reputationScore).toFixed(0)}/100
                                        </span>
                                    )}
                                    {strategy.leader?.followersCount !== undefined && (
                                        <span>{strategy.leader.followersCount.toLocaleString()} followers</span>
                                    )}
                            ...
                            {strategy.leader && (
                                <div
                                    style={{ color: '#7461FF', fontFamily: 'Space Grotesk', fontSize: '13px', cursor: 'pointer' }}
                                    onClick={() => navigate(`/social/profile/${strategy.leader!.id}`)}
                                >
                                    View Profile →
                                </div>
                            )}
