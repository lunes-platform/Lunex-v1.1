...
const pulse = keyframes`
  0%, 100% { box-shadow: 0 0 0 0 rgba(108, 56, 255, 0.4); }
  50%       { box-shadow: 0 0 0 10px rgba(108, 56, 255, 0); }
`
...
const AgentCard = styled.div<{ active?: boolean; accentColor: string }>`
  background: #1E1E1E;
  border: 1px solid ${({ active, accentColor }) => active ? accentColor + '66' : '#2A2A2C'};
  border-radius: 18px;
  padding: 24px;
  cursor: pointer;
  transition: all 0.2s;
...
const StepCircle = styled.div<{ active: boolean; done: boolean }>`
  ...
  border-radius: 50%;
  border: 2px solid ${({ active, done }) => done ? '#6C38FF' : active ? '#6C38FF' : '#2A2A2C'};
  background: ${({ active, done }) => done ? '#6C38FF' : active ? 'rgba(108,56,255,0.12)' : '#1E1E1E'};
  display: flex; align-items: center; justify-content: center;
  font-family: 'Space Grotesk', sans-serif;
  font-size: 16px;
  font-weight: 700;
...
const StepBtn = styled.button<{ primary?: boolean }>`
  ...
  &:hover {
    background: ${({ primary }) => primary ? '#7D4DFF' : 'rgba(255,255,255,0.04)'};
    ...
    <Page>
      <Container>
        ...
        <Hero>
          ...
          <HeroTitle>
            Your Bot, Your <span>Strategy</span>,<br />Your Edge.
          </HeroTitle>
          <HeroSub>
            Agents are autonomous trading entities on Lunex. Connect a human wallet,
            an AI model, an OpenClaw bot, or a custom algo — and publish strategies
            to the marketplace in minutes.
          </HeroSub>
          <HeroCTA onClick={() => navigate('/agent')}>Get Started — Create Agent</HeroCTA>
        </Hero>
          ...
          <StepperTrack>
            {STEPS.map((s, i) => (
              <React.Fragment key={i}>
                <Step active={activeStep === i} done={activeStep > i} onClick={() => setActiveStep(i)}>
                  <StepCircle active={activeStep === i} done={activeStep > i}>
                    {activeStep > i ? '✓' : i + 1}
                  </StepCircle>
                  <StepLabel active={activeStep === i}>{s.label}</StepLabel>
                </Step>
                {i < STEPS.length - 1 && <StepConnector done={activeStep > i} />}
              </React.Fragment>
            ))}
          </StepperTrack>
          ...
          <StepContent>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <StepBtn primary onClick={() => navigate(step.ctaPath)}>{step.cta}</StepBtn>
                {activeStep < STEPS.length - 1 && (
                  <StepBtn onClick={() => setActiveStep(activeStep + 1)}>Next Step →</StepBtn>
                )}
                {activeStep > 0 && (
                  <StepBtn onClick={() => setActiveStep(activeStep - 1)}>← Back</StepBtn>
                )}
              </div>
            </div>
          </StepContent>
