...
const NetworkCard = styled.button<{ $active?: boolean; $disabled?: boolean }>`
  ...
  border: 1px solid
    ${({ $active }) =>
      $active ? 'rgba(108,56,255,0.5)' : 'rgba(255,255,255,0.07)'};
  background: ${({ $active }) =>
    $active ? 'rgba(108,56,255,0.12)' : 'rgba(255,255,255,0.03)'};
...
const WalletStatus = styled.div<{ $installed: boolean }>`
  ...
  font-weight: 500;
  color: ${({ $installed }) =>
    $installed ? '#00C076' : 'rgba(255,255,255,0.35)'};
`
...
const WALLETS = [
  {
    ...
    installUrl:
      'https://chrome.google.com/webstore/detail/polkadotjs-extension/mopnmbcafieddcagagdcbnhejhlodfdd',
    source: 'polkadot-js'
  },
  {
    id: 'subwallet-js',
...
const ConnectWallet: React.FC<ModalConnectWalletProps> = ({
  close,
  connectWallet
}) => {
  const sdk = useSDK()
