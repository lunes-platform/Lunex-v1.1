...
function toPlancks(units: string, decimals = 12): string {
    try {
        ...
        return (BigInt(intPart) * PLANCKS_PER_UNIT + BigInt(padded || '0')).toString()
    } catch {
        return '0'
    }
}
...
export function useAsymmetricDeploy() {
    const { walletAddress, isConnected } = useSDK()
    ...
    const getAccount = useCallback(async (): Promise<InjectedAccountWithMeta | null> => {
    ...
    const deploy = useCallback(
        async (args: DeployStrategyArgs, contractAddress: string) => {
            ...
            if (!account) {
                setState((s) => ({ ...s, step: 'error', error: 'Could not find wallet account' }))
                return
            }
            ...
            try {
                ...
                const body = {
                    contractAddress,
                    pairSymbol: args.pairSymbol,
                    baseToken: args.baseToken,
                    quoteToken: args.quoteToken,
                    ownerAddress: walletAddress,
                    autoRebalance: args.autoRebalance ?? false,
                    profitTargetBps: args.profitTargetBps ?? 300,
                    buyCurve: {
                        gamma: args.buyGamma,
                        maxCapacity: args.buyMaxCapacity,
                        feeBps: args.buyFeeBps,
                    },
                    sellCurve: {
                        gamma: args.sellGamma,
                        maxCapacity: args.sellMaxCapacity,
                        feeBps: args.sellFeeBps,
                    },
                }
                ...
                if (!res.ok) {
                    ...
                    throw new Error((err as { error?: string }).error ?? 'Backend registration failed')
                }
        ...
        [isConnected, walletAddress, getAccount],
    )
    ...
    const reset = () => setState({ step: 'idle', txHash: null, contractAddress: null, strategyId: null, error: null })
