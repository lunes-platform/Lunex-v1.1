...
const WEBMCP_CDN = 'https://unpkg.com/@anthropic-ai/webmcp@latest/dist/webmcp.min.js'
    ...
    const init = () => {
      ...
      mcp.registerTool(
        ...
        'Navigate to a specific page of the Lunex DEX. Available pages: swap, spot, pools, staking, rewards, copytrade, agents, strategies, governance, docs, affiliates.',
        {
          page: {
            type: 'string',
            description: 'Page name: swap, spot, pools, staking, rewards, copytrade, agents, strategies, governance, docs, affiliates',
          },
        },
        (args: { page: string }) => {
      ...
      mcp.registerResource(
        ...
        () => ({
          contents: [{
            ...
            text: JSON.stringify({
              routes: Object.entries(APP_ROUTES).map(([name, path]) => ({ name, path })),
              externalLinks: [
