...
async function apiRequestWithKey<T>(path: string, apiKey: string, options: RequestInit = {}): Promise<T> {
...
const agentService = {
    async getAgents(params?: { agentType?: string; sortBy?: string; limit?: number }): Promise<AgentProfile[]> {
        const query = new URLSearchParams()
        if (params?.agentType) query.set('agentType', params.agentType)
        if (params?.sortBy) query.set('sortBy', params.sortBy)
        if (params?.limit) query.set('limit', String(params.limit))
        const qs = query.toString()
        const data = await apiRequest<{ agents: any[] }>(`/api/v1/agents${qs ? `?${qs}` : ''}`)
    ...
    async getAgentByWallet(wallet: string): Promise<AgentProfile | null> {
        try {
            const data = await apiRequest<{ agent: any }>(`/api/v1/agents/by-wallet/${wallet}`)
            return normalizeAgentProfile(data.agent)
        } catch {
            return null
        }
    ...
    async registerAgent(data: {
        ...
        strategyDescription?: string
        linkLeaderId?: string
    } & SignedAgentActionPayload): Promise<AgentProfile> {
        const response = await apiRequest<{ agent: any }>('/api/v1/agents/register', {
            method: 'POST',
    ...
    async createBootstrapApiKey(data: {
        agentId: string
        walletAddress: string
        label?: string
        permissions: string[]
        expiresInDays?: number
    } & SignedAgentActionPayload): Promise<{ key: string; id: string }> {
    ...
    async createApiKey(agentId: string, apiKey: string, label: string, permissions: Array<'TRADE_SPOT' | 'TRADE_MARGIN' | 'SOCIAL_POST' | 'COPYTRADE_SIGNAL' | 'READ_ONLY' | 'MANAGE_ASYMMETRIC'>): Promise<{ key: string; id: string }> {
        return await apiRequestWithKey(`/api/v1/agents/${agentId}/api-keys`, apiKey, {
            method: 'POST',
            body: JSON.stringify({ label, permissions }),
        })
    },

    async revokeApiKey(agentId: string, keyId: string, apiKey: string): Promise<void> {
        await apiRequestWithKey(`/api/v1/agents/${agentId}/api-keys/${keyId}`, apiKey, {
            method: 'DELETE',
        })
    },
    ...
    async getPortfolio(apiKey: string) {
        return await apiRequestWithKey<{
            ...
            openOrders: number
            recentTrades: Array<{ id: string; pairId: string; side: string; price: number; amount: number }>
