...
const INSTANTIATE_GAS = {
  refTime: BigInt('200000000000'),
  proofSize: BigInt('4000000')
}
...
class AsymmetricContractService {
      ...
      tx.signAndSend(
        ...
        (result: any) => {
          if (result.dispatchError) {
            reject(
              new Error(
                'Instantiation failed: ' + result.dispatchError.toString()
              )
            )
            return
          }
          if (result.status.isInBlock || result.status.isFinalized) {
            ...
            if (instantiatedEvent) {
              ...
              if (process.env.NODE_ENV !== 'production') {
                console.log('[instantiate] deployed contract:', contractAddress)
              }
              resolve(contractAddress)
            } else if (result.status.isFinalized) {
              reject(
                new Error(
                  'Contract instantiated but Instantiated event not found for this account'
                )
              )
            }
          }
  ...
  async getBuyCurve(
    ...
    caller: string
  ): Promise<CurveSide | null> {
    try {
      const c = this.getContract(contractAddress)
      const { result, output } = await c.query.getBuyCurve(caller, {
  ...
  async getOwner(
    ...
    try {
      const c = this.getContract(contractAddress)
      const { output } = await c.query.getOwner(caller, {
        gasLimit: this.makeDryGas()
      })
  ...
  async deployLiquidity(
    ...
    if (output) {
      ...
      if (errVariant !== undefined && errVariant !== null) {
        ...
        const storedOwner = await this.getOwner(
          contractAddress,
          account.address
        )
        console.error(
    ...
    return await new Promise<string>((resolve, reject) => {
      c.tx
        .deployLiquidity(
          { gasLimit: execGas, storageDepositLimit: null },
          buyK,
          sellK
        )
        .signAndSend(
    ...
    return await new Promise<string>((resolve, reject) => {
      c.tx
        .updateCurveParameters(
          { gasLimit: gasRequired as any, storageDepositLimit: null },
          ...args
        )
        .signAndSend(
          account.address,
