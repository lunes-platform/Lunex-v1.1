...
function normalize(s: any): Strategy {
  return {
    ...
    leader: s.leader
      ? {
          ...s.leader,
          roi30d: Number(s.leader.roi30d ?? 0),
          followersCount: Number(s.leader.followersCount ?? 0)
        }
      : undefined
  }
}
...
const strategyService = {
  ...
  async getPerformanceHistory(
    ...
    const data = await apiRequest<{ history: any[] }>(
      `/api/v1/strategies/${id}/performance?days=${days}`
    )
  ...
  async followStrategy(
    ...
    const auth = await this.signStrategyAction(
      'strategies.follow',
      followerAddress,
      signMessage,
      {
    ...
    return apiRequest<{ following: boolean; followersCount: number }>(
      ...
      {
        method: 'POST',
        body: JSON.stringify({ followerAddress, allocatedCapital, ...auth })
      }
    )
  },
  ...
  async unfollowStrategy(
    ...
    return apiRequest<{ following: boolean; followersCount: number }>(
      ...
      {
        method: 'DELETE',
        body: JSON.stringify({ followerAddress, ...auth })
      }
    )
  },

  async getFollowedStrategies(
    address: string,
    signMessage: (message: string) => Promise<string>
  ): Promise<Strategy[]> {
    const auth = await this.signStrategyAction(
