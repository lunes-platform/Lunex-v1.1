...
export function multiplyAmount(amount: string, multiplier: number): string {
    const result = BigInt(amount) * BigInt(Math.floor(multiplier * 10000)) / BigInt(10000);
...
export function convertDecimals(
    ...
    if (fromDecimals < 0 || fromDecimals > 18 || toDecimals < 0 || toDecimals > 18) {
...
export function validateSwapDecimals(
    ...
    if (decimalDiff >= 10) {
        warnings.push(`Large decimal difference (${decimalDiff}) - verify amounts carefully`);
    }

    // Warn about low decimal output tokens
    if (decimalsOut <= 2) {
        warnings.push(`Output token has low decimals (${decimalsOut}) - small amounts may be truncated`);
    }
