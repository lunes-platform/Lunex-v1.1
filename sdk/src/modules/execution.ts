...
export class ExecutionModule {
  ...
  async getHistory(params?: {
    ...
    const response = await this.http.get<{
      ...
      total: number;
    }>('/api/v1/execution/history', params);

    return {
      logs: response.logs ?? response.entries ?? [],
  ...
  async updateStatus(
    ...
    const response = await this.http.patch<{
      log?: ExecutionLog;
      entry?: ExecutionLog;
    }>(`/api/v1/execution/${logId}/status`, { status, executedAmount });
    return response.log ?? (response.entry as ExecutionLog);
  }
}
