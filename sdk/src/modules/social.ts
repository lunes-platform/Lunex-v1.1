...
export class SocialModule {
  ...
  async getStats(): Promise<SocialStats> {
    const response = await this.http.get<{ stats: SocialStats }>(
      '/api/v1/social/stats',
    );
  ...
  async getLeaders(params?: {
    ...
    const response = await this.http.get<{ leaders: SocialLeader[] }>(
      '/api/v1/social/leaders',
      params,
    );
  ...
  async getLeaderboard(limit = 10): Promise<SocialLeader[]> {
    const response = await this.http.get<{ leaderboard: SocialLeader[] }>(
  ...
  async getLeaderProfile(
    ...
    const response = await this.http.get<{ leader: SocialLeader }>(
      `/api/v1/social/leaders/${leaderId}`,
      { viewerAddress },
    );
  ...
  async getIdeas(limit = 50): Promise<SocialIdea[]> {
    const response = await this.http.get<{ ideas: SocialIdea[] }>(
      '/api/v1/social/ideas',
      { limit },
    );
  ...
  async followLeader(
    leaderId: string,
    input: SocialSignedAddressActionInput,
  ): Promise<SocialFollowResult> {
  ...
  async commentOnIdea(
    ideaId: string,
    input: SocialSignedCommentInput,
  ): Promise<SocialIdeaComment> {
    const response = await this.http.post<{ comment: SocialIdeaComment }>(
