...
export class StrategyModule {
    ...
    async get(strategyId: string): Promise<Strategy> {
        const response = await this.http.get<{ strategy: Strategy }>(`/api/v1/strategies/${strategyId}`);
        return response.strategy;
    }

    /** Get performance history (requires no auth) */
    async getPerformance(strategyId: string, days = 30): Promise<StrategyPerformancePoint[]> {
        const response = await this.http.get<{ history: StrategyPerformancePoint[] }>(
    ...
    async create(input: CreateStrategyInput): Promise<Strategy> {
        const response = await this.http.post<{ strategy: Strategy }>('/api/v1/strategies', input);
        return response.strategy;
    }

    /** Update a strategy — requires agent API key */
    async update(strategyId: string, input: UpdateStrategyInput): Promise<Strategy> {
        const response = await this.http.patch<{ strategy: Strategy }>(`/api/v1/strategies/${strategyId}`, input);
        return response.strategy;
    }

    /** Follow a strategy */
    async follow(strategyId: string, followerAddress: string, allocatedCapital?: number): Promise<{ following: boolean; followersCount: number }> {
        return this.http.post(`/api/v1/strategies/${strategyId}/follow`, { followerAddress, allocatedCapital });
    }

    /** Unfollow a strategy */
    async unfollow(strategyId: string, followerAddress: string): Promise<{ following: boolean; followersCount: number }> {
        return this.http.delete(`/api/v1/strategies/${strategyId}/follow`, { followerAddress });
    }

    /** Get strategies followed by a wallet */
    async getFollowed(walletAddress: string): Promise<Strategy[]> {
        const response = await this.http.get<{ followed: Array<{ strategy: Strategy }> }>(
    ...
    async syncPerformance(strategyId: string): Promise<{ synced: boolean; performance?: StrategyPerformancePoint }> {
        return this.http.post(`/api/v1/strategies/${strategyId}/sync-performance`, {});
    }
}
