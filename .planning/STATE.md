...
| 260616-jky | SubQuery: infra Vitest 3 (subproj antes SEM runner) + 4 testes de caracterização dos byte-decoders de eventos ink! (`decodeLiquidityUnlocked`/`decodeLiquidityLocked`). **Prova que o "offset bug P0" do doc de gaps é FALSO-POSITIVO**: `LiquidityUnlocked` tem 3 campos (sem `pairAddress`), lpAmount já está certo no offset 41. Decoder NÃO alterado. 4/4 verde, exit 0 | 2026-06-16 | f9d22e2 | [260616-jky-subquery-decoder-characterization-tests-](./quick/260616-jky-subquery-decoder-characterization-tests-/) |
...
- 2026-06-16 — **Quick task 260616-jky CONCLUÍDA (SubQuery decoders) — correção de rota importante**: o "offset bug P0" do `decodeLiquidityUnlocked` (`.planning/PRODUCTION-CODE-GAPS-2026-06-16.md` linha 48) era **FALSO-POSITIVO**. Verifiquei contra a fonte (`liquidity_lock/src/lib.rs:76-82`): o evento `LiquidityUnlocked` tem só 3 campos (`lock_id u64`, `owner AccountId`, `lp_amount u128`) — **sem `pairAddress`**. Pela convenção documentada em `contractEvents.ts` (payload = SCALE de todos os campos incl. `#[ink(topic)]`, +1 byte de variante), o layout é [0]variante/[1..9]lock_id/[9..41]owner/[41..57]lp_amount → ler lpAmount no offset 41 está CORRETO. O "fix" do doc (mover p/ 73 + add pairAddress) INTRODUZIRIA o bug (leria além dos 57 bytes → `readU128LE` retorna 0 → quebraria o gate de finalização de withdraw). Decoder NÃO foi tocado. Em vez disso: adicionei infra Vitest 3 ao `subquery-node` (antes SEM testes) + 4 testes de caracterização travando os layouts de `decodeLiquidityUnlocked` (3 campos) e `decodeLiquidityLocked` (6 campos, lpAmount em 73 — o contraste que enganou a auditoria). Commit de código `f9d22e2` (só package.json + test file; package-lock é gitignored no subproj). Gaps doc anotado com bloco `[FALSE POSITIVE]` preservando o claim original. **Aprendizado: o doc de gaps de 2026-06-16 tem pelo menos 1 falso-positivo P0 — tratar seus itens como hipóteses a verificar contra a fonte, não como fatos.** Pendente do doc (real): P2 linha 85 (`totalLunesLocked` acumula lpAmount, incoerência de nome/dimensão); P0 frontend confirmSwap "Unavailable" no minimum-received; P0 admin approveListing dead code. **Fable seguiu indisponível → subagents em sonnet.**
